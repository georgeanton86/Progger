...
export function ScopeValidationTab() {
  ...
  async function validate(scenario?: string) {
    ...
    const res = await fetch("/api/ai", {
      ...
      body: JSON.stringify({
        prompt: `Evaluate this clinical scenario for scope of practice compliance. Provide:
1. Verdict: Within Scope / Caution / Outside Scope
2. Risk Level: Low / Medium / High
3. Regulatory Analysis (state-specific considerations)
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        <div className="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          <button onClick={() => validate()} disabled={loading || !input.trim()} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {presets.map(s => (
            <button key={s} onClick={() => { setInput(s); validate(s); }} className="text-left p-3 bg-gray-800 hover:bg-gray-700 rounded-lg text-xs text-gray-300 transition-colors">{s}</button>
          ))}
        </div>
      </div>
